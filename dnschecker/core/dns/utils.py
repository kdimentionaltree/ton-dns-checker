...
def encode_domain(domain):
    ...
    for ch in domain:
        code = ord(ch)
        if code <= 32:
            raise ValueError('bytes in range 0..32 are not allowed in domain names')
        if code >= 127 and code <= 159:
            raise ValueError('bytes in range 127..159 are not allowed in domain names')
